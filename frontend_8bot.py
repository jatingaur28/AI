...
# ═══════════════════════════════════════════════════════════════
# 2. CONSTANTS (RESTORED TO FIX NAME ERROR)
# ═══════════════════════════════════════════════════════════════
AMBER  = "#ffb300"
GREEN  = "#00e676"
RED    = "#ff1744"
...
PURPLE = "#ea80fc"
BG     = "rgba(0,0,0,0)"
BGCARD = "rgba(10, 13, 20, 0.7)"
BORDER = "#131722"

# ═══════════════════════════════════════════════════════════════
# 3. DATA LAYER & SECTOR MAPPING
# ═══════════════════════════════════════════════════════════════
...
def _mock_report() -> dict:
    now = datetime.now()
    total = 1_000_000
    lt_eq, st_eq, tr_eq = total * 0.60 + 15500, total * 0.25 + 6200, total * 0.15 + 8100
    def port(name, eq, base, trades, wr, pf, sh, dd, grade, w):
        return {
            ...
            "open_positions": {s: {"qty": random.randint(10, 100), "ltp": random.uniform(200, 3500)} for s in random.sample(ALL_SYMBOLS[:30], random.randint(3, 6))},
        }
    return {
...
# ═══════════════════════════════════════════════════════════════
# 4. SIDEBAR CONTROLS & DYNAMIC POSITION SIZER SANDBOX
# ═══════════════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    # Kelly Formula calculation: f* = (p*b - (1-p)) / b
    ...
    <div style='background:rgba(6,8,13,0.9); border:1px solid #131722; padding:12px; border-radius:6px; font-size:11px; line-height:1.7;'>
        ...
        <div style='color:var(--text-mid);'>Half-Kelly (Safe): <span style='color:var(--green); font-weight:700;'>{half_kelly:.1%}</span></div>
        <div style='color:var(--text-mid);'>Max Position Sizing: <span style='color:var(--amber); font-weight:700;'>₹{optimal_allocation:,.0f}</span></div>
    </div>
    """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
# 5. ZERO-FLICKER OMNI-PAGE DASHBOARD
# ═══════════════════════════════════════════════════════════════
...
def render_dashboard(current_sym, user_calc_alloc):
    ...
    <div style='display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:15px;'>
        <div>
            ...
            <div style='font-family:"Bebas Neue"; font-size:68px; color:{GREEN}; text-shadow:0 0 30px {GREEN}80; line-height:1;'>
                ₹{current_total_equity:,.0f}
            </div>
        </div>
        <div style='text-align:right'>
            <div class='live-dot'></div><span style='color:{GREEN}; font-size:13px; font-weight:700; letter-spacing:2px;'>SYSTEM SYNCHRONIZED</span>
    ...
    # Consolidated Active Positions Table
    all_positions = []
    for pname, pdata in ports.items():
        for sym, pos_info in pdata.get("open_positions", {}).items():
            ...
            all_positions.append({
                ...
                "Book Weight": exposure / max(pdata.get("equity", 1), 1),
                "Status": "PROFIT" if random.random() > 0.3 else "LOSS"
            })
    ...
    if all_positions:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Book Weight": st.column_config.ProgressColumn(format="%.1f%%", min_value=0, max_value=1),
    ...
    with tape_col:
        ...
        for _ in range(8):
            ...
            tape_rows.append(f"<div style='display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px solid #131722; font-size:11px;'><span style='color:#8a90a0;'>{now_str}</span><span style='color:{color}; font-weight:700;'>{side}</span><span>₹{base_price*(1+random.uniform(-0.001,0.001)):.2f}</span><span style='color:#ffffff;'>{size} sh</span><span style='color:var(--amber); font-size:10px;'>[{tag}]</span></div>")
    ...
    # Mathematical vote breakdown table
    factors = [
    ...
    # 2. AI Intelligence Log & Recommendations
    with g2:
        ...
        for rec in report.get("recommendations", []):
            st.markdown(f"<div style='background:rgba(10,13,20,0.8); border-left:3px solid var(--amber); padding:10px 14px; margin-bottom:8px; border-radius:0 4px 4px 0; font-size:11px;'>{rec}</div>", unsafe_allow_html=True)
